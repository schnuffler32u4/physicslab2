...
plt.errorbar(xdata, ydata, xerr=xerr, yerr=yerr, c='g')
plt.scatter(xdata, ydata, label='Measurements', c='g', s=15, marker="^")
...
plt.plot(newxqr, functions.voltdis(newxqr, *functions.letters(0.1, 0.1, 57, 7, 1e-10), functions.mutual(0.35)), 'r-',
         label='Prediction')
plt.errorbar(newx, newy, xerr=newxerror, yerr=newyerror, c='g')
plt.scatter(newx, newy, label='Measurements', c='g', s=15, marker="^")
# plt.plot(xdata, functions.voltvar(xdata, 0.0409, 0.086, 57, 7, 1.106e-10), label='Fit but with extracted RLC values')
plt.xlabel('ω [rad/s]')
plt.ylabel('Peak voltage [V]')
plt.legend(loc="upper right", borderaxespad=0.5)
...
plt.plot(xqr, functions.voltnores(xqr, *norespopt), 'r-', label='Fit')
plt.errorbar(xdata, ydata, xerr=xerr, yerr=yerr, c='g')
plt.scatter(xdata, ydata, label='Measurements', c='g', s=15, marker="^")
plt.xlabel('ω [rad/s]')
plt.ylabel('Peak voltage [V]')
plt.legend(loc="upper right", borderaxespad=0.5)
plt.title("Fit parameters: \n L1 = 0.022±0.004 H, L2 = 0.0913±0.0003 H, \n C1 = 1.4±0.2e-10 F")
...
print(
    f"with L1 = {norespopt[0]}±{functions.roundup(np.sqrt(np.diag(norespcov))[0])} H, L2 = {norespopt[1]}±{functions.roundup(np.sqrt(np.diag(norespcov))[1])} H, C1 = {norespopt[2]}±{functions.roundup(np.sqrt(np.diag(norespcov))[2])} F")
plt.plot(newxqr, functions.voltnores35(newxqr, *norespopt), 'r-', label='Fit')
plt.errorbar(newx, newy, xerr=newxerror, yerr=newyerror, c='g')
plt.scatter(newx, newy, label='Measurements', c='g', s=15, marker="^")
plt.xlabel('ω [rad/s]')
plt.ylabel('Peak voltage [V]')
plt.legend(loc="upper right", borderaxespad=0.5)
plt.title("Fit parameters: \n L1 = 0±10000 H, L2 = 0±400 H, \n C1 = 0±8e-6")
...
plt.plot(xqr, functions.voltvar(xqr, *popt), 'r-', label='Fit' )
plt.errorbar(xdata, ydata, xerr=xerr, yerr=yerr, c='g')
plt.scatter(xdata, ydata, label='Measurements', c='g', s=15, marker="^")
plt.xlabel('ω [rad/s]')
plt.ylabel('Peak voltage [V]')
plt.legend(loc="upper right", borderaxespad=0.5)
plt.title("Fit parameters: \n L1 = 0.0403±0.0002 H, L2 = 0.08715±0.00002 H, \n R1 = 1067±3 Ω, R2 = 1034±5 Ω, "
...
functions.M = functions.mutual(0.35)
plt.plot(newxqr, functions.voltvar(newxqr, *popt), 'r-', label='Fit')
plt.errorbar(newx, newy, xerr=newxerror, yerr=newyerror, c='g')
plt.scatter(newx, newy, label='Measurements', c='g', s=15, marker="^")
plt.xlabel('ω [rad/s]')
plt.ylabel('Peak voltage [V]')
plt.legend(loc="upper right", borderaxespad=0.5)
plt.title("Fit parameters: \n L1 = 0.055±0.002 H, L2 = 0.06435±0.00005 H, \n R1 = 940±20 Ω, R2 = 910± 20 Ω, "
          "\n C1 = 1.23±0.03e-10 F")
plt.savefig("Fitting the values for a distance of 35 cm.jpg", dpi=500)
plt.show()

p, = plt.plot(xqr, functions.voltdis(xqr, *functions.letters(*a), functions.mutual(0.05)), 'r-')
plt.xlabel("Angular frequency [arbritrary units]")
plt.ylabel("Voltage [Arbitraty units]")
plt.ylim(0, 100)
...
plt.plot(xdis, functions.v1dis(xdis, 4, 0.1, 57, 7, 1e-10), 'g-', label='My own choosing of parameters')
...
plt.plot(xfr, functions.f1dis(xfr, 0.1, 0.1, 57, 7, 1e-10), 'g-',
         label='Parameters of my choosing')
plt.xlabel('Distance [cm]')
plt.ylabel('First peak angular frequency [rad/s]')
plt.legend()
plt.show()'''

res = 100  # the resolution of the line used for the fit
...
functions.double_plot(bcv_d13, bcf_d13, bcv13, bcf13, bcv_d13_err, bcf_d13_err, bcv13_err, bcf13_err, dist,
                      ...
                      functions.v1disres(dist / 10, 0.1, 0.1, 57, 1000, 1e-10) / np.sqrt(2), "first, merged",
                      "resistor")
